...
### 1. 后端服务（linkbot-ai 仓库）

**仓库：** `light618/linkbot-ai`

这个仓库包含两个服务，需要在 Railway 中创建**两个服务**：

#### 1.1 Go 代理服务（渠道监听）

在 Railway 项目中创建第一个服务：
- **根目录：** `/`（根目录）
- **构建命令：** Railway 会自动检测 Go 项目
...
- **根目录：** `/backend`
- **构建命令：** Railway 会自动检测 Node.js 项目

**环境变量：**
```bash
API_PORT=3001
CORS_ORIGIN=https://你的前端地址.railway.app
PROXY_URL=https://你的go服务地址.railway.app  # 后端调用Go服务的地址
# 数据库和Redis配置...
```

### 2. 前端服务（linkbot-ai-frontend 仓库）
...
- [ ] Go 服务已部署并运行（linkbot-ai 仓库，根目录）
- [ ] Node.js 后端已部署并运行（linkbot-ai 仓库，backend/ 目录）
- [ ] React 前端已部署并运行（linkbot-ai-frontend 仓库）
- [ ] 所有环境变量已配置
- [ ] `PROXY_URL` 环境变量指向 Go 服务地址
- [ ] 抖音回调地址已在抖音后台配置
...
**两个代码库，三个服务：**
1. **linkbot-ai** 仓库 → 部署为两个服务：
   - Go 服务（根目录 `/`）
   - Node.js 后端（`/backend` 目录）
2. **linkbot-ai-frontend** 仓库 → 部署为一个服务：
   - React 前端（根目录 `/`）
...
- Node.js 后端通过 HTTP 调用 Go 服务，需要分开部署
