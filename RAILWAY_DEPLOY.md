...
本项目包含三个服务，需要在 Railway 上分别部署：

1. **Go 代理服务** (`linkbot-ai/`) - 渠道消息监听和转发
2. **Node.js 后端 API** (`linkbot-ai/backend/`) - 业务逻辑和数据库操作
3. **React 前端** (`linkbot-ai-frontend/`) - 用户界面
...
#### 2.1 创建新服务
- 在同一个 Railway 项目中，点击 "New Service"
- 选择 "Deploy from GitHub repo"
- 选择 `light618/linkbot-ai` 仓库
- 设置根目录为 `backend/`
